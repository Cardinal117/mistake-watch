...
4. Recommendations come before real waveform architecture because they rely on existing metadata and queue behavior rather than a new playback-analysis stack.
5. Real waveform architecture comes after listen/preload stability because it can affect playback, performance, and mobile behavior.
6. Avatar motion stays after core room features because it is identity polish, not room functionality.
7. R2, voting, accounts/friends, browser mode, and hardening remain later because they are larger systems with storage, auth, security, or infrastructure consequences.
